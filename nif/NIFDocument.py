    ...
    while i<len(nifStatements):
        if nifStatements[i].startswith('<http'):
            ...
            while not contComplete:
                if re.search('nif:beginIndex[ \t]+"([0-9]+)"\^\^xsd:nonNegativeInteger',nifStatements[i]):
